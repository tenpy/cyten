...
///
/// @param operator_ The data to be converted to a Coupling as a backend-specific block or some data that can be converted using `as_block`. The order of axes must match the `sites`, that is, the axes correspond to ``[p0, p1, ..., p1*, p0*]`` (codomain legs ascending, domain legs descending), where ``pi`` corresponds to site ``sites[i]``. The block should be given in the "public" basis order of the sites, i.e., according to ``sites[i].sectors_of_basis``.
/// @param sites The sites that the operators act on.
/// @param name A descriptive name that can be used when pretty-printing, to identify the coupling.
/// @param dtype If given, the block is converted to that dtype and the resulting tensors in the factorization will have that dtype. By default, we detect the dtype from the block.
/// @param cutoff_singular_values If given, truncate singular values (see `horizontal_factorization`) below this threshold.
...
///
/// @param operator_ Operator to be converted to a coupling. The legs should be ordered as ``[p0, p1, ..., p1*, p0*]``, where ``pi`` and ``pi*`` correspond to the legs associated with site ``sites[i]``.
/// @param sites The sites that the operator acts on.
/// @param name A descriptive name that can be used when pretty-printing, to identify the coupling. For example, a Heisenberg coupling is usually initialized with name ``'S.S'``.
/// @param cutoff_singular_values If given, truncate singular values (see `horizontal_factorization`) below this threshold.
...
///
/// @param all_sites The sites the returned coupling should be based on. Only the positions defined by `coupling_positions` are used.
/// @param coupling_positions Strictly ascending, one entry per `factorization` tensor: the index into `all_sites` where that tensor should sit.
...
///
/// @param permutation A permutation of ``range(len(self.sites))``. ``permutation[k]`` is the index (in `self`'s current order) of the site that ends up at new position `k`.
/// @param levels One entry per site of `self` (in `self`'s current order): its "height", used to derive the braid chirality (over/under) for any elementary transposition whose `over_braid` entry is ``None``, the same way `Symmetry` legs with a higher level braid over those with a lower one. Only needed for symmetries without a symmetric braid (see `braiding_style`); ignored otherwise.
/// @param over_braid One entry per elementary adjacent-site transposition needed to realize `permutation` (i.e. NOT one entry per site -- the number of transpositions depends on `permutation`, e.g. via the number of its inversions). Explicitly fixes the braid chirality for that transposition (``True`` = the site moving from the lower position over the one moving from the higher position); ``None`` derives it from `levels`.
/// @returns A new coupling with `sites` (and the represented operator) reordered according to `permutation`.
...
///
/// @param sites The sites that the coupling acts on. Note that the order matters for the final leg order.
...
/// @param species If given, the chemical potential only couples to the occupation of this species. By default, it couples to the total occupation of all species.
...
/// @param species If given, we use only the occupation of this one species as the density @f$ n_i @f$. By default, we use the total occupation of all species.
...
/// @param species_i, species_j If given, we use only the occupation of this one species as the density @f$ n_{i/j} @f$. By default, we use the total occupation of all species. Note that if the two species are different, this coupling alone is not hermitian!
...
/// @param species Which species should participate (the sum above goes over ``k_i, k_j in zip(*species)``). By default, we let @f$ k_i = k_j @f$ go over all species, i.e. include all "species preserving" hoppings.
...
/// @param species Which species should participate (the sum above goes over ``k_i, k_j in zip(*species)``). By default, we let @f$ k_i = k_j @f$ go over all species, i.e. include all "same-species" pairings.
/// onsite_pairing
...
/// @param sites The sites that the coupling acts on. Note that the order matters for the final leg order.
/// @param Delta Prefactor, as given above. By default ``1``.
/// @param species Which species should participate (the sum above goes over ``k_1, k_2 in zip(*species)``). By default, we let @f$ k_1 = k_2 @f$ go over all species, i.e. include all "same-species" pairings.
/// pairing
...
///
/// @param sites The sites that the coupling acts on. Note that the order matters for the final leg order.
/// @param J Prefactor, as given above. By default ``1``. Positive `J` energetically favor the trivial fusion channel, i.e. they are the "antiferromagnetic" analog.
