...
enum class FusionStyle
{
    ...
    multiple_unique =
      10,         // every sector appears at most once in pairwise fusion, N^{ab}_c \in {0,1}
...
///
/// Product symmetries limit the size of each of the Sectors to a fixed bit-length such that sum of
/// bit_lengths <= 64.
...
template<int N>
Sector _compress_sector_fixed(std::array<Sector, N> decompressed,
                              std::array<int, N> const& bit_lengths);

std::vector<Sector> _decompress_sector(Sector compressed, std::vector<int> const& bit_lengths);

template<int N>
std::array<Sector, N> _decompress_sector_fixed(Sector compressed,
                                               std::array<int, N> const& bit_lengths);
...
class Symmetry
{
  public:
    ...
    size_t _max_sector_bits =
      -1; // how many bits are needed to represent biggest sector in this int?
  ...
  public:
    ...
    /// dimension computed by :meth:`qdim`. For other braiding styles,
