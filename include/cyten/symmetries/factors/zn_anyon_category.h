...
/// Abelian anyon category :math:`Z_N^{(n)}` (fusion rules of ``Z_N``).
///
/// Allowed sectors are 1D arrays with a single integer in ``[0, N-1]``.
class ZNAnyonCategory : public SymmetryFactor
{
  public:
    using Ptr = std::shared_ptr<ZNAnyonCategory>;
    using CPtr = std::shared_ptr<const ZNAnyonCategory>;

    int N;
    int n;
    complex128 _phase;

    ZNAnyonCategory(int N, int n, std::optional<std::string> descriptive_name = std::nullopt);
    ~ZNAnyonCategory() override = default;
