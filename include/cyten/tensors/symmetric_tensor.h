...
/// @param domain The domain. ``None`` (the default) is equivalent to ``[]``, i.e. no legs in the domain.
...
/// @param labels Can either specify the labels for all legs of the resulting tensor, like in the constructor of `SymmetricTensor`. Alternatively, can give labels only for the codomain (one list), and the domain labels are constructed as their dual labels i.e. ``'p' <-> 'p*'``.
/// @param backend The backend of the tensor.
...
///
/// @param trees Specifies the linear combination that defines the resulting tensor. Each entry of the dict, ``{(X, Y): coeffs}`` represents several contributions to the linear combination, one per entry of the block ``coeffs``. The contribution with prefactor ``coeffs[n1, ..., nJ, mK, ..., m1]`` (note the axis order!) consists of the following steps as a map from domain to codomain::  1. Project each leg ``k`` of the domain to a single sector, where the sector is given by ``Y.uncoupled[k]`` and the degeneracy index by ``mk`` (an index to the array ``coeffs``).  2. Apply the fusion tree ``Y``.  3. Apply the splitting tree ``X``.  4. Apply inclusions on each leg ``j`` of the codomain, where the sector is given by ``X.uncoupled[j]`` and the degeneracy index by ``nj`` (an index to the array ``coeffs``).
/// @param codomain, domain, backend, labels Arguments, like for constructor of `SymmetricTensor`.
