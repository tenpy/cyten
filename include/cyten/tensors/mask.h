...
///
/// @param data The numerical data (i.e. boolean flags) comprising the mask. type is backend-specific. Should have boolean dtype.
/// @param space_in The single space of the domain. This is the large leg for projections or the small leg for inclusions.
/// @param space_out The single space of the codomain This is the small leg for projections or the large leg for inclusions.
/// @param is_projection If this Mask is a projection (from large to small) map. Otherwise it is in inclusion map (from small to large). Required if ``space_in == space_out``, since it is ambiguous in that case.
/// @param backend The backend of the tensor.
/// @param labels Specify the labels for the legs. Can either give two lists, one for the codomain, one for the domain. Or a single flat list for all legs in the order of the `legs`, such that ``[codomain_labels, domain_labels]`` is equivalent to ``[*codomain_labels, *reversed(domain_labels)]``.
///
/// Notes:
///
...
/// leg appear in order.
///
/// @param block_mask A boolean Block indicating for each basis element of the public basis, if it is kept.
...
/// that are kept appear in order.
///
/// @param indices Valid index/indices for a 1D numpy array. The elements of the public basis of `large_leg` with these indices are kept by the projection.
...
///
/// @param large_leg The large leg, in the domain of the projection
/// @param small_leg The small leg. If given, must be a subspace of the `large_leg` with compatible basis order (see notes in class docstring of `Mask`). If ``None``, a small leg is randomly generated, according to `p_keep` and `min_keep`.
/// @param backend, labels Arguments, like for the constructor
/// @param p_keep If `small_leg` is not given, the probability that any single sector is kept. Is ignored if `small_leg` is given, since it determines the number of kept sectors.
/// @param min_keep If `small_leg` is not given, the minimum number of sectors kept. Is ignored of `small_leg` is given.
