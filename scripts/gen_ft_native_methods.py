#!/usr/bin/env python3
"""Generate native C++ replacements for delegated FusionTreeBackend methods."""

from pathlib import Path

OUT = Path(__file__).resolve().parents[1] / "src/backends/_ft_native_methods.inc"
...
{
    py::object res = ft_py().attr("_partial_trace_helper")(py::cast(tree), idcs);
    auto tup = res.cast<py::tuple>();
    return { tup[0].cast<bool>(), tup[1].cast<complex128>() };
}
                                   ...
                                   TensorProduct::Ptr new_domain)
{
    ...
    for (py::ssize_t r = 0; r < b_buf.shape(0); ++r)
        b_sectors.append(b.attr("domain").attr("sector_decomposition").attr("__getitem__")(b_buf(r, 1)));
    auto eff_space = ElementarySpace::from_defining_sectors(b.attr("symmetry").cast<Symmetry::Ptr>(), b_sectors);
    ...
    std::map<std::string, TrafoMap> tree_transformations;
    ...
    for (py::ssize_t block_ind = 0; block_ind < a_buf.shape(0); ++block_ind) {
        ...
        for (auto const& tb : iter_space->iter_tree_blocks(coupled_arr)) {
            ...
            auto vl = tb.tree.vertex_labels(std::max<int64>(0, leg_idx - 1));
            std::string dummy_key;
            for (auto const& s : { std::get<0>(vl), std::get<1>(vl), std::get<2>(vl), std::get<3>(vl) })
                dummy_key += s.str() + "|";
...
# Continue with partial_trace, from_tree_pairs, get_element, from_grid, scale_axis, mask methods
# Due to length, append in parts
...
# Fix partial_compose eff_space and cache key in CODE - patch via replace in script
CODE = CODE.replace(
    "    py::list b_sectors;\n    auto b_bi = py::array_t<int64, py::array::c_style | py::array::forcecast>::ensure(b_data->block_inds);\n    auto b_buf = b_bi.unchecked<2>();\n    for (py::ssize_t r = 0; r < b_buf.shape(0); ++r)\n        b_sectors.append(b.attr(\"domain\").attr(\"sector_decomposition\").attr(\"__getitem__\")(b_buf(r, 1)));\n    auto eff_space = ElementarySpace::from_defining_sectors(b.attr(\"symmetry\").cast<Symmetry::Ptr>(), b_sectors);",
...
OUT.write_text(CODE + PART2 + PART3)
