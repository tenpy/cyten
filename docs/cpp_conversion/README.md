...
7. **Docstrings** — Dual-language policy (see [tenpy/cyten#242](https://github.com/tenpy/cyten/issues/242)): shared API semantics live in `///` on headers (NumPy/reST, C++ types). Full Python surface docs stay in pybind `R"pydoc(...)"` for wrappers/lambdas (`py::object`, label parsing, `None` defaults). 1:1 `&T::m` / free-function bindings may use `DOC(cyten, …)` from pybind11_mkdoc once wired (pilot: `ops_algebra.h`). Indent `R"pydoc(` content with the surrounding `.def()` block. The [.cursor/skills/pybind11-codegen/](.cursor/skills/pybind11-codegen/) script still copies Python docstrings into binding drafts.
