...
7. **Docstrings** — Dual-language policy (see [tenpy/cyten#242](https://github.com/tenpy/cyten/issues/242)): shared API semantics live in `///` on headers (NumPy/reST, C++ types). Full Python surface docs stay in pybind `R"pydoc(...)"` for wrappers/lambdas. 1:1 bindings may use `DOC(cyten, …)` from [pybind/docstrings.h](../../pybind/docstrings.h) (generated from `ops_algebra.h` by pybind11_mkdoc; **checked into git**). Regeneration is optional (`CYTEN_GENERATE_DOCSTRINGS`, needs pybind11-mkdoc + clangdev + libclang); without those tools the build uses the committed header. Indent `R"pydoc(` with the surrounding `.def()` block.
