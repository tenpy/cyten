...
def setup_benchmark(mod_q=[1], sectors=3, size=20, legs=2, select_frac=1.0, dtype=np.float64, **kwargs):
    """Hermitian map with matching (co)domain, for timing ``ct.eigh``."""
    symmetry = symmetry_from_mod_q(mod_q)
    backend = get_backend_from_kwargs(symmetry, **kwargs)
    spaces = [random_elementary_space(symmetry, size, sectors) for _ in range(legs)]
    ...
    A = random_tensor(
        ...
        permute=False,  # eigh needs domain == codomain
    )
    return A + A.hc


def benchmark(data):
    ct.eigh(data, new_labels=['e'], new_leg_dual=False)
