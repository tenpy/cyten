...
Call this file with arguments, e.g,::
    python benchmark.py -m tensordot_cyten tensordot_numpy -l 2 -s 20 -q 1 1
...
fn_template = '{mod_name!s}_benchmark_s_{sectors:d}_l_{legs:d}_mod_q_{mod_q_str}_{symmetry_backend}_{block_backend}.txt'
...
def _add_common_args(parser):
    parser.add_argument(
        ...
        nargs='*',
        default=[],
        help='Nature of the charge. Empty: no symmetry. 1: U(1). n>=2: Z_n. Length = number of factors.',
    )
    ...
    parser.add_argument(
        '--symmetry-backend',
        dest='symmetry_backend',
        default='abelian',
        help='Cyten symmetry backend (e.g. abelian, no_symmetry, fusion_tree).',
    )
    parser.add_argument(
        '--block-backend',
        dest='block_backend',
        default='numpy',
        help='Cyten block backend (numpy or torch).',
    )
    return parser
...
if __name__ == '__main__':
    ...
    kwargs = dict(
        ...
        symmetry_backend=args.symmetry_backend,
