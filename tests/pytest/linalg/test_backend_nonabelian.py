...
from conftest import random_tensor, random_vector_space
...
def test_nonabelian_partial_trace(symmetry: Symmetry, block_backend: str,
    ...
    for i in range(num_legs):
        if i in idcs2:
            ...
            spaces.append(random_vector_space(symmetry=symmetry, np_random=np_random))
...
def cross_check_partial_trace(ten: SymmetricTensor, pairs: list[tuple[int, int]], levels: list[int]
                              ) -> tuple[fusion_tree_backend.FusionTreeBackend, ProductSpace, ProductSpace]:
...
def cross_check_single_c_symbol_tree_cols(ten: SymmetricTensor, leg: int | str, levels: list[int]
    ...
    for tree, slc, _ in ftb._tree_block_iter_product_space(iter_space, iter_coupled):
