...
def check_to_tensor(op: sparse.LinearOperator, vec: Tensor):
    ...
    res_tensor = tensor.tdot(vec, range(vec.num_legs, 2 * vec.num_legs),
                             reversed(range(vec.num_legs)))
...
@pytest.mark.parametrize(['penalty', 'project_operator'], [(None, True), (2. - .3j, True),
                                                           (-4, False)])
def test_ProjectedLinearOperator(make_compatible_tensor, penalty, project_operator):
    ...
    projected_op = sparse.ProjectedLinearOperator(original_op, [o1, o2],
...
def test_NumpyArrayLinearOperator_sector(make_compatible_space,
    ...
    #
    sectors = tensors.detect_sectors_from_block(psi0_np.reshape([a.dim, b.dim]),
                                                legs=[a, b],
                                                backend=get_backend(symmetry=a.symmetry,
                                                                    block_backend='numpy'))
    sector = a.symmetry.fusion_outcomes(*sectors)[0, :]
    #
    if use_hermitian:
        H_op = sparse.HermitianNumpyArrayLinearOperator.from_Tensor(H,
                                                                    legs1=['a*', 'b*'],
                                                                    legs2=['a', 'b'],
                                                                    charge_sector=sector)
    else:
        H_op = sparse.NumpyArrayLinearOperator.from_Tensor(H,
                                                           legs1=['a*', 'b*'],
                                                           legs2=['a', 'b'],
                                                           charge_sector=sector)
    psi_init = tensors.ChargedTensor.random_uniform(legs=[a, b],
                                                    charge=sector,
                                                    labels=['a', 'b'],
                                                    charged_state=[1])
