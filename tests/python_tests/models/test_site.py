...
def test_site(np_random, block_backend, symmetry_backend, use_sym):
    ...
    s = sites.Site(leg, backend=backend, state_labels=labels)
    ...
    op2 = cyten.SymmetricTensor.random_uniform([leg, leg.dual], backend, labels=['p', 'p*'])
    ...
    assert isinstance(s.get_op('op3'), cyten.DiagonalTensor)
...
def test_double_site(any_backend):
    if isinstance(any_backend, cyten.NoSymmetryBackend):
        all_conserve = ['None']
        fs_conserve = 'None'
    elif isinstance(any_backend, (cyten.AbelianBackend, cyten.FusionTreeBackend)):
...
def check_operator_availability(s: sites.Site, expect_symmetric_ops: dict[str, bool],
    ...
    for name, is_diag in expect_symmetric_ops.items():
        ...
        assert isinstance(s.symmetric_ops[name], cyten.DiagonalTensor) == is_diag
...
def test_spin_half_site(any_backend):
    ...
    elif isinstance(any_backend, cyten.FusionTreeBackend):
    ...
    for conserve in all_conserve:
        ...
        s = sites.SpinHalfSite(conserve, backend=any_backend)
        assert cyten.almost_equal(s['Sp'], s['Sm'].hconj())
        s.test_sanity()
...
def test_spin_site(any_backend, S):
    ...
    for conserve in all_conserve:
        ...
        s = sites.SpinSite(S, conserve, backend=any_backend)
        s.test_sanity()
        for op in s.all_op_names:
            assert s.hc_ops[op] == hcs[op]
...
def test_fermion_site(any_backend):
    ...
    sites = []
    for conserve in all_conserve:
        s = sites.FermionSite(conserve, backend=any_backend)
        s.test_sanity()
        for op in s.all_op_names:
            assert s.hc_ops[op] == hcs[op]
...
def test_spin_half_fermion_site(any_backend):
    ...
    for conserve_N, conserve_S in it.product(all_conserve_N, all_conserve_S):
        ...
        s = sites.SpinHalfFermionSite(conserve_N, conserve_S, backend=any_backend)
        s.test_sanity()
        for op in s.all_op_names:
            assert s.hc_ops[op] == hcs[op]
        Id, JW = s.Id.to_numpy(), s.JW.to_numpy()
...
def test_spin_half_hole_site(any_backend):
    ...
    for conserve_N, conserve_S in it.product(all_conserve_N, all_conserve_S):
        print(f'{conserve_N=}, {conserve_S=}')
        s = sites.SpinHalfHoleSite(conserve_N, conserve_S, backend=any_backend)
        s.test_sanity()
        for op in s.all_op_names:
            assert s.hc_ops[op] == hcs[op]
        JW = s.JW.to_numpy()
...
def test_boson_site(any_backend, Nmax):
    ...
    for conserve in all_conserve:
        ...
        if isinstance(any_backend, backends.FusionTreeBackend):
            with pytest.raises(NotImplementedError, match='diagonal_from_block_func not implemented'):
                s = sites.BosonSite(Nmax, conserve=conserve, backend=any_backend)
            return  # TODO
...
def test_clock_site(any_backend, q):
    ...
    for conserve in all_conserve:
        ...
        if isinstance(any_backend, backends.FusionTreeBackend):
            with pytest.raises(NotImplementedError, match='diagonal_from_block_func not implemented'):
                s = sites.ClockSite(q=q, conserve=conserve, backend=any_backend)
            return  # TODO
...
def test_set_common_symmetry(any_backend):
    if isinstance(any_backend, cyten.NoSymmetryBackend):
        ...
        expect_symm_Sz = expect_symm_Sz_N = expect_symm_Sz_N_Sz = cyten.no_symmetry
    else:
        conserve_S = 'Sz'
        conserve_N = 'N'
    ...
    if isinstance(any_backend, backends.FusionTreeBackend):
        with pytest.raises(NotImplementedError, match='diagonal_from_block_func not implemented'):
            spin = sites.SpinSite(S=0.5, conserve=conserve_S, backend=any_backend)
        return  # TODO

    spin = sites.SpinSite(S=0.5, conserve=conserve_S, backend=any_backend)
    spin1 = sites.SpinSite(S=1, conserve=conserve_S, backend=any_backend)
    ferm = sites.SpinHalfFermionSite(conserve_N=conserve_N, conserve_S=conserve_S)
    boson = sites.BosonSite(Nmax=2, conserve=conserve_N, backend=any_backend)
    ...
    for symmetry_combine, expect_symm in [('by_name', expect_symm_Sz_N),
                                          ...
                                          ('drop', cyten.no_symmetry)]:
        print(f'{symmetry_combine=}')
        sites.set_common_symmetry([spin, ferm], symmetry_combine=symmetry_combine)
    ...
    sites.set_common_symmetry([ferm, spin], symmetry_combine=lambda i, s: s[:, :1],
    ...
    some_sites = [ferm, spin1, spin, boson]
    ...
    sites.set_common_symmetry(some_sites, [spin_half_Sz, weird_occupation, spin_one_Sz], new_symmetry)
