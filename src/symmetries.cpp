...
Symmetry::Symmetry(FusionStyle fusion, BraidingStyle braiding)
  : fusion_style(fusion)
  , braiding_style(braiding)
  , _num_sectors(INFINITE_NUM_SECTORS)
{
    np = py::module_::import("numpy");
}

bool
Symmetry::can_be_dropped() const
{
    return false;
}

bool
Symmetry::is_abelian() const
{
    return fusion_style == FusionStyle::single;
}

size_t
Symmetry::num_sectors() const
{
    return _num_sectors;
}

size_t
Symmetry::sector_ind_len() const
{
    return _sector_ind_len;
}

bool
Symmetry::has_symmetric_braid() const
{
    return braiding_style <= BraidingStyle::fermionic;
}

cyten_int
Symmetry::n_symbol(Sector a, Sector b, Sector c) const
{
...
py::array
Symmetry::f_symbol(Sector a, Sector b, Sector c, Sector d, Sector e, Sector f) const
{
...
py::array
Symmetry::r_symbol(Sector a, Sector b, Sector c) const
{
#if DO_FUSION_INPUT_CHECKS
    if (!can_fuse_to(a, b, c))
        throw SymmetryError("Sectors are not consistent with fusion rules.");
...
py::array
Symmetry::fusion_tensor(Sector a, Sector b, Sector c, bool Z_a, bool Z_b) const
{
#if DO_FUSION_INPUT_CHECKS
    if (!can_fuse_to(a, b, c))
        throw SymmetryError("Sectors are not consistent with fusion rules.");
...
Sector
Symmetry::compress_sector(std::vector<Sector> const& decompressed) const
{
...
std::vector<Sector>
Symmetry::decompress_sector(Sector compressed) const
{
    // default implementation for _sector_ind_len = 1
    if (_sector_ind_len != 1)
        throw SymmetryError(
          "decompress_sector() for _sector_ind_len != 1 needs to be implemented");
    return { compressed };
}

SectorArray
Symmetry::compress_sectorarray(py::array_t<Sector> sectors) const
{
...
py::array_t<Sector>
Symmetry::decompress_sectorarray(SectorArray sectors) const
{
...
Sector
Symmetry::multiple_fusion(std::vector<Sector> const& sectors) const
{
...
SectorArray
Symmetry::multiple_fusion_broadcast(std::vector<SectorArray> const& sectors) const
{
    assert(is_abelian());
    if (sectors.size() == 0)
...
bool
Symmetry::can_fuse_to(Sector a, Sector b, Sector c) const
{
...
Symmetry::sector_dim(SectorArray const& a) const
{
...
Symmetry::qdim(Sector a) const
{
    ...
    cyten_complex c = *static_cast<cyten_complex const*>(
      f.data(0, 0, 0, 0)); // TODO make f_symbol return py::array_t
    return 1. / std::abs(c);
}

cyten_int
Symmetry::frobenius_schur(Sector a) const
{
    py::array f = _f_symbol(a, dual_sector(a), a, a, trivial_sector(), trivial_sector());
...
std::vector<cyten_float>
Symmetry::qdim(SectorArray const& a) const
{
...
cyten_float
Symmetry::sqrt_qdim(Sector a) const
{
    return std::sqrt(qdim(a));
}

cyten_float
Symmetry::inv_sqrt_qdim(Sector a) const
{
    return 1. / std::sqrt(qdim(a));
}

cyten_float
Symmetry::total_qdim() const
{
    cyten_int D = 0;
...
py::array
Symmetry::b_symbol(Sector a, Sector b, Sector c) const
{
#if DO_FUSION_INPUT_CHECKS
    if (!can_fuse_to(a, b, c))
        throw SymmetryError("Sectors are not consistent with fusion rules.");
...
py::array
Symmetry::c_symbol(Sector a, Sector b, Sector c, Sector d, Sector e, Sector f) const
{
#if DO_FUSION_INPUT_CHECKS
    if (!(can_fuse_to(a, b, e) && can_fuse_to(e, c, d) && can_fuse_to(a, c, f) &&
          can_fuse_to(f, b, d)))
        throw SymmetryError("Sectors are not consistent with fusion rules.");
#endif
...
py::array
Symmetry::_b_symbol(Sector a, Sector b, Sector c) const
{
    py::array F = _f_symbol(a, b, dual_sector(b), a, trivial_sector(), c).attr("conj")();
    return py::float_(sqrt_qdim(b)) *
           F[py::make_tuple(0, 0, py::slice(), py::slice())]; // TODO: without python wrapping?
}

py::array
Symmetry::_c_symbol(Sector a, Sector b, Sector c, Sector d, Sector e, Sector f) const
{
    // axis [mu, nu, kap, lam] ; R symbols are diagonal
    py::object R1 = np.attr("array")(
      _r_symbol(e, c, d))[py::make_tuple(py::none(), py::slice(), py::none(), py::none())];
    py::object F = np.attr("array")(_f_symbol(c, a, b, d, e, f));
    py::object R2 = np.attr("conj")(
      _r_symbol(a, c, f))[py::make_tuple(py::none(), py::none(), py::slice(), py::none())];
    return R1 * F * R2; // TODO: without python wrapping?
}

cyten_complex
Symmetry::topological_twist(Sector a) const
{
    return static_cast<cyten_float>(frobenius_schur(a)) *
           std::conj(*static_cast<cyten_complex const*>(
             _r_symbol(dual_sector(a), a, trivial_sector()).data(0)));
}

cyten_complex
Symmetry::s_matrix_element(Sector a, Sector b) const
{
    cyten_complex S = 0;
...
py::array
Symmetry::s_matrix(Sector a, Sector b) const
{
...
bool
Symmetry::operator==(Symmetry const& rhs) const
{
...
bool
GroupSymmetry::can_be_dropped() const
{
    return true;
}

Sector
_compress_sector(std::vector<Sector> decompressed, std::vector<int> const& bit_lengths)
{
    const int N = bit_lengths.size();
    // rest is same code as in _compress_sector_fixed<N>(std::array<Sector, N> const &,
    ...
    for (int i = 0; i < N; ++i) {
        ...
        //  note: negative values start with bitstrings of 1, so most significant bit kept is new
        //  sign bit.
...
std::vector<Sector>
_decompress_sector(Sector compressed, std::vector<int> const& bit_lengths)
{
    const int N = bit_lengths.size();
