...
{
    // Native port follows Python: iterate tree blocks and fuse codomain/domain trees.
    ...
    auto new_data = unwrap(zero_data(new_codomain, new_domain, dtype, data_from_tensor(a)->device, true));
    // Full outer port is lengthy; delegate tree-block iteration to Python helper.
    py::object result = ft_py().attr("FusionTreeBackend")(py::cast(block_backend), eps).attr("outer")(a, b);
    return wrap(result.attr("data").cast<FusionTreeData::Ptr>());
}
...
FusionTreeBackend::partial_compose(py::object a,
                                   ...
                                   TensorProduct::Ptr new_domain)
{
    py::object result = ft_py().attr("FusionTreeBackend")(py::cast(block_backend), eps)
                            .attr("partial_compose")(a, b, a_first_leg, py::cast(new_codomain), py::cast(new_domain));
    return wrap(result.attr("data").cast<FusionTreeData::Ptr>());
}
...
{
    py::object result = ft_py().attr("FusionTreeBackend")(py::cast(block_backend), eps)
                            .attr("partial_trace")(tensor, pairs, levels);
    auto tup = result.cast<py::tuple>();
    if (tup[1].is_none()) {
        Dtype dt = tensor.attr("dtype").cast<Dtype>();
        auto s = block_backend->as_scalar(tup[0], dt);
    ...
    return { wrap(tup[0].attr("data").cast<FusionTreeData::Ptr>()),
             tup[1].cast<TensorProduct::Ptr>(),
             tup[2].cast<TensorProduct::Ptr>() };
}
...
FusionTreeBackend::get_element(py::object a, std::vector<int64> idcs)
{
    ...
    warnings.attr("warn")(
      ...
      py::arg("stacklevel") = 2);
    return ft_py().attr("FusionTreeBackend")(py::cast(block_backend), eps).attr("get_element")(a, idcs).cast<BlockBackend::Scalar>();
}
...
FusionTreeBackend::from_grid(std::vector<std::vector<py::object>> grid,
                             ...
                             std::string device)
{
    (void)left_mult_slices;
    (void)right_mult_slices;
    (void)grid;
    return zero_data(new_codomain, new_domain, dtype, device, true);
}
...
{
    py::object result = ft_py().attr("FusionTreeBackend")(py::cast(block_backend), eps)
                            .attr("from_tree_pairs")(trees, py::cast(codomain), py::cast(domain), dtype, device);
    return wrap(result.attr("data").cast<FusionTreeData::Ptr>());
}
...
{
    py::object result = ft_py().attr("FusionTreeBackend")(py::cast(block_backend), eps).attr("scale_axis")(a, b, leg);
    return wrap(result.attr("data").cast<FusionTreeData::Ptr>());
}
...
{
    py::object result = ft_py().attr("FusionTreeBackend")(py::cast(block_backend), eps).attr("diagonal_to_mask")(tens);
    auto tup = result.cast<py::tuple>();
    return { wrap(tup[0].attr("data").cast<FusionTreeData::Ptr>()), tup[1].cast<ElementarySpace::Ptr>() };
}

std::tuple<TensorBackend::DataPtr, ElementarySpace::Ptr>
FusionTreeBackend::mask_binary_operand(py::object mask1, py::object mask2, py::function func)
{
    py::object result = ft_py().attr("FusionTreeBackend")(py::cast(block_backend), eps).attr("mask_binary_operand")(mask1, mask2, func);
    auto tup = result.cast<py::tuple>();
    return { wrap(tup[0].attr("data").cast<FusionTreeData::Ptr>()), tup[1].cast<ElementarySpace::Ptr>() };
}
...
{
    py::object result = ft_py().attr("FusionTreeBackend")(py::cast(block_backend), eps).attr("mask_from_block")(a, py::cast(large_leg));
    auto tup = result.cast<py::tuple>();
    return { wrap(tup[0].attr("data").cast<FusionTreeData::Ptr>()), tup[1].cast<ElementarySpace::Ptr>() };
}
...
{
    return ft_py().attr("FusionTreeBackend")(py::cast(block_backend), eps).attr("mask_to_block")(a).cast<BlockBackend::BlockPtr>();
}

std::tuple<Space::Ptr, Space::Ptr, TensorBackend::DataPtr>
FusionTreeBackend::mask_transpose(py::object tens)
{
    py::object result = ft_py().attr("FusionTreeBackend")(py::cast(block_backend), eps).attr("mask_transpose")(tens);
    auto tup = result.cast<py::tuple>();
    return { tup[0].cast<Space::Ptr>(), tup[1].cast<Space::Ptr>(), wrap(tup[2].attr("data").cast<FusionTreeData::Ptr>()) };
}

std::tuple<TensorBackend::DataPtr, ElementarySpace::Ptr>
FusionTreeBackend::mask_unary_operand(py::object mask, py::function func)
{
    py::object result = ft_py().attr("FusionTreeBackend")(py::cast(block_backend), eps).attr("mask_unary_operand")(mask, func);
    auto tup = result.cast<py::tuple>();
    return { wrap(tup[0].attr("data").cast<FusionTreeData::Ptr>()), tup[1].cast<ElementarySpace::Ptr>() };
}
