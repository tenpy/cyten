...
bind_block_backend(py::module_& m)
{
    ...
    // gen_pyb11_binding --py-name BlockBackend --header-file include/cyten/block_backend.h
    // --binding-file pybind/block_backend.cpp

    py::class_<BlockBackend, py::smart_holder> block_backend(m, "BlockBackend");
    block_backend.doc() = "Abstract base class that defines the operation on dense blocks.";

    block_backend // init and attributes
      .def(py::init<std::string>())
      .def_readonly_static("BlockCls",
                           &BlockBackend::_get_BlockCls); // completed block_backend attributes

    block_backend //  methods
      .def("__repr__", &BlockBackend::__repr__)
      .def("__str__", &BlockBackend::__str__)
      .def("abs",
           &BlockBackend::abs,
           py::arg("a"),
