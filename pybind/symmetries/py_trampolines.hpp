...
// CHECKME: the following was generated by .cursor/skills/pybind11-codegen/pybind11_codegen.py gen_pyb11_trampoline --py-name Leg --header-file include/cyten/symmetries/spaces.h --binding-file pybind/symmetries/py_trampolines.hpp

/// @brief pybind11 trampoline class for Leg in Python
template<class LegBase=Leg>
class PyLeg : public LegBase, py::trampoline_self_life_support {
    public:
        using LegBase::LegBase; // inherit constructors

        Ptr dual() override {
            PYBIND11_OVERRIDE_PURE(Ptr, LegBase, dual);
        }
        bool is_trivial() override {
            PYBIND11_OVERRIDE_PURE(bool, LegBase, is_trivial);
        }
        void set_basis_perm(std::optional<std::vector<int64>> basis_perm) override {
            PYBIND11_OVERRIDE(void, LegBase, set_basis_perm,
                basis_perm);
        }
        std::vector<Ptr> flat_legs() override {
            PYBIND11_OVERRIDE(std::vector<Ptr>, LegBase, flat_legs);
        }
        std::vector<Ptr> flat_spaces() override {
            PYBIND11_OVERRIDE(std::vector<Ptr>, LegBase, flat_spaces);
        }
        void set_inverse_basis_perm(std::optional<std::vector<int64>> inverse_basis_perm) override {
            PYBIND11_OVERRIDE_NAME(void, LegBase, "inverse_basis_perm", set_inverse_basis_perm,
                inverse_basis_perm);
        }
        int64 num_flat_legs() override {
            PYBIND11_OVERRIDE(int64, LegBase, num_flat_legs);
        }
        std::string ascii_arrow() override {
            PYBIND11_OVERRIDE(std::string, LegBase, ascii_arrow);
        }
        void test_sanity() override {
            PYBIND11_OVERRIDE(void, LegBase, test_sanity);
        }
        py::object as_Space() override {
            PYBIND11_OVERRIDE_PURE(py::object, LegBase, as_Space);
        }
        py::object as_ElementarySpace(bool is_dual) override {
            PYBIND11_OVERRIDE(py::object, LegBase, as_ElementarySpace,
                is_dual);
        }
        Ptr dual() override {
            PYBIND11_OVERRIDE_PURE(Ptr, LegBase, dual);
        ...
        std::vector<int64> _flat_leg_permutation(int64 offset) override {
            PYBIND11_OVERRIDE(std::vector<int64>, LegBase, _flat_leg_permutation,
                offset);
        }
        std::string ascii_arrow() override {
            PYBIND11_OVERRIDE(std::string, LegBase, ascii_arrow);
        }
        bool operator==(const Leg & other) override {
            PYBIND11_OVERRIDE_PURE_NAME(bool, LegBase, "__eq__", operator==,
                other);
        }
};  // trampoline class PyLeg
